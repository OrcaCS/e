...
public class PrimaryController {
    ...
    private void onDraw(ActionEvent event) { // MAYBE ADD A MOUSE CURSOR WITH CIRCLE?
        imageView.setStyle("-fx-border-color: red; -fix-background-color: rgba)255,0,0,0.2");
        drawOn = true;
        ...
        Image image = imageView.getImage(); // gets current image already there
        WritableImage writable = new WritableImage(image.getPixelReader(), width, height);
        imageView.setImage(writable); // this chunk is to stop the draw method from ending on one click without saving
        
        imageView.getParent().setMouseTransparent(true);
        imageView.setPickOnBounds(true);
        imageView.setMouseTransparent(false);
        imageView.setFocusTraversable(true);
        imageView.toFront();
        imageView.setOnMousePressed(e -> System.out.println("imageview got click"));

        Node parent = imageView.getParent();
        if (parent != null) {
            parent.setPickOnBounds(true);
            parent.setMouseTransparent(false);
            parent.setOnMousePressed(e -> System.out.println("parent got click"));
        }
        ...
        System.out.println("mousepressed hander: " + imageView.getOnMousePressed());
    }
    ...
    private void handleMousePressed(MouseEvent event) {
        if (!(imageView.getImage() instanceof WritableImage)) {
            System.out.println("image is not writable");
            return;
        }
        ...
        if (centerX < 0 || centerX >= width || centerY < 0 || centerY >= height) {
            System.out.println("mouse position outside image: " + centerX + "," + centerY);
            return;
        }

        System.out.println("mouse at: " + centerX + "," + centerY);
        System.out.println("Before: " + reader.getColor(centerX, centerY));

        int startX = Math.max(centerX - RADIUS, 0);
        int endX = Math.min(centerX + RADIUS, 0);
        int startY = Math.max(centerY - RADIUS, 0);
        int endY = Math.min(centerY + RADIUS, 0);

        for (int x = startX; endX <= endX; x++) {
            for (int y = startY; endY <= endY; y++) {
